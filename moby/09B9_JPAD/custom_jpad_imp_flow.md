...
For now, we hijack the Snowman Skin (16DB oClass) which has Primary 0. 
...
Why not our own? Perhaps a new class can be thrown into the registry rows, but I am afraid of growing the size and moving targets. 
  0x3BB100  registry rows
  ...
  0x3BB76C  sentinel row: FFFFFFFF 00000000 003BB778
  0x3BB778  secondary descriptor blobs start here
...
Instead, we immediatley force jump state 6 and drive a vertical impulse. This can be buffed by ranging the gravity constant, which will gracefully and naturally reset on state change. 
